...
// sorting
(function(){
    const button = $('.js-sorting-button'),
          map = $('.js-sorting-map');
...
(function(){
    ...
    button.on('click', function () {
        const guestsNumber = Number($('.js-counter-quests').val());
        guestsNumber > 0
            ? guestsNumber > 1
                ? content.text(guestsNumber + " guests")
                : content.text(guestsNumber + " guest")
            : "1 guest";
    });
}());
    ...
    if (range.length) {
        range.each(function(){
            ...
            slider[0].noUiSlider.on('update', function( values, handle ) {
                if (handle) {
                    let textFinish = [values[handle]];
                    finish.text(time(textFinish));
                } else {
                    let textStart = [values[handle]];
                ...
                getValue: function()
                    {
                        if ($('.js-date-from').val() && $('.js-date-to').val() )
                            return $('.js-date-from').val() + $('.js-date-to').val();
                        else
                            return '';
                    },
...
// dateSinglePicker
(function(){
    const dateSingle = $('.js-date-single');
...
// faq
(function () {
    const item = $('.faq__item, .faq1__item'),
...
// upload preview show
(function(){
    const preview = $('.js-upload-preview'),
